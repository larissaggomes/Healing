...
def cadastro_medico(request):
    ...
    if request.method == "GET":
        ...
        messages.add_message(request, constants.SUCESS, 'Cadastro medico realizado com sucesso!')
        return redirect('/medicos/abrir_horario')
...
def abrir_horario(request):
    ...
    if request.method == "GET":
        ...
        if data_formatada <= datetime.now():
            messages.add_message(request, constants.WARNING, 'A data nao pode ser anterior a data atual')
            return redirect('/medicos/abrir_horario')
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        return render(request, 'consulta_area_medico.html', {'consulta': consulta, 'documento': documento,'is_medico': is_medico(request.user)})
    elif request.method == "POST":
